...
class sale_order(osv.osv):
    ...
    #HIEUND add following to update quotation
    def button_dummy(self, cr, uid, ids, context=None):

        for order in self.browse(cr, uid, ids, context=context):
            res =  self.pool.get('sale.order.line').update_price(cr,uid,order.order_line,pricelist=order.pricelist_id.id,date_order=order.date_order,
                                                          partner= order.partner_id.id,context = context)
            if res:
                raise osv.except_osv('warning','Cannot update price from pricelist')
...
class sale_order_line(osv.osv):
    ...
    #HieuND adds function to update price when price lis
    def update_price(self,cr,uid,ids,pricelist,date_order,partner=None,context=None):
        context = context or {}
        result = {}
        warning_msgs = ''
        for line_obj in ids:
            product = self.pool.get('product.product').browse(cr,uid,line_obj.product_id.id,context)
            uom = self.pool.get('product.uom').browse(cr,uid,line_obj.product_uom.id,context)
            qty = line_obj.product_uom_qty        
            _logger.warning('product: %s, uom: %s, qty: %s', product,uom, qty)
            if not pricelist:
                warn_msg = _('You have to select a pricelist or a customer in the sales form !\n'
                    'Please set one before choosing a product.')
                warning_msgs += _("No Pricelist ! : ") + warn_msg +"\n\n"
                ...
                price = self.pool.get('product.pricelist').price_get(cr, uid, [pricelist],
                     product.id, qty or 1.0, partner, {
                        'uom': uom.id or result.get('product_uom'),
                        'date': date_order,
                        })[pricelist]
                if price is False:
                    warn_msg = _("Cannot find a pricelist line matching this product and quantity.\n"
                    ...
                    result = self.write(cr,uid,line_obj.id,{'price_unit': price})
            warning= {}
        if warning_msgs:
            warning = {
                   'title': _('Configuration Error!'),
