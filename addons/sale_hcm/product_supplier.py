...
class product_supplier_category(osv.osv):
    ...
    def name_get(self, cr, uid, ids, context=None):
        ...
        if context.get('partner_category_display') == 'short':
            return super(product_supplier_category, self).name_get(cr, uid, ids, context=context)
    ...
    _columns = {
        ...
        'parent_id': fields.many2one('product.supplier.category', 'Parent Category', select=True, ondelete='cascade'),
        'complete_name': fields.function(_name_get_fnc, type="char", string='Full Name'),
        'child_ids': fields.one2many('product.supplier.category', 'parent_id', 'Child Categories'),
        'active': fields.boolean('Active', help="The active field allows you to hide the category without removing it."),
        'parent_left': fields.integer('Left parent', select=True),
        'parent_right': fields.integer('Right parent', select=True),
        'partner_ids': fields.many2many('product.supplier', id1='category_id', id2='partner_id', string='Partners'),
    }
    _constraints = [
...
class product_supplier_title(osv.osv):
    _name = 'product.supplier.title'
    _order = 'name'
    _columns = {
...
class product_supplier(osv.osv, format_address):
    ...
    _columns = {
        ...
        'title': fields.many2one('product.supplier.title', 'Title'),
        'parent_id': fields.many2one('product.supplier', 'Related Company'),
        'child_ids': fields.one2many('product.supplier', 'parent_id', 'Contacts', domain=[('active','=',True)]), # force "active_test" domain to bypass _search() override    
        ...
        'bank_ids': fields.one2many('product.supplier.bank', 'partner_id', 'Banks'),
        'website': fields.char('Website', size=64, help="Website of Partner or Company"),
        'comment': fields.text('Notes'),
        'category_id': fields.many2many('product.supplier.category', id1='partner_id', id2='category_id', string='Tags'),
        ...
        'image_medium': fields.function(_get_image, fnct_inv=_set_image,
            ...
            store={
                'product.supplier': (lambda self, cr, uid, ids, c={}: ids, ['image'], 10),
            },
        ...
        'commercial_partner_id': fields.function(_commercial_partner_id, type='many2one', relation='product.supplier', string='Commercial Entity')
    }
    ...
    def fields_view_get(self, cr, user, view_id=None, view_type='form', context=None, toolbar=False, submenu=False):
        ...
        res = super(product_supplier,self).fields_view_get(cr, user, view_id, view_type, context, toolbar=toolbar, submenu=submenu)
        if view_type == 'form':
    ...
    _defaults = {
        ...
        'company_id': lambda self, cr, uid, ctx: self.pool.get('res.company')._company_default_get(cr, uid, 'product.supplier', context=ctx),
        'color': 0,
        'is_company': False,
    ...
    def copy(self, cr, uid, id, default=None, context=None):
        ...
        return super(product_supplier, self).copy(cr, uid, id, default, context)
    ...
    def _check_ean_key(self, cr, uid, ids, context=None):
        for partner_o in pooler.get_pool(cr.dbname).get('product.supplier').read(cr, uid, ids, ['ean13',]):
...
#   _constraints = [(_check_ean_key, 'Error: Invalid ean code', ['ean13'])]
    ...
    def update_address(self, cr, uid, ids, vals, context=None):
        ...
        if addr_vals:
            return super(product_supplier, self).write(cr, uid, ids, addr_vals, context)
    ...
    def write(self, cr, uid, ids, vals, context=None):
        if isinstance(ids, (int, long)):
            ids = [ids]
        result = super(product_supplier,self).write(cr, uid, ids, vals, context=context)
        for partner in self.browse(cr, uid, ids, context=context):
            self._fields_sync(cr, uid, partner, vals, context)
        return result

    def create(self, cr, uid, vals, context=None):
        new_id = super(product_supplier, self).create(cr, uid, vals, context=context)
    ...
    def _search(self, cr, user, args, offset=0, limit=None, order=None, context=None, count=False, access_rights_uid=None):
        ...
        return super(product_supplier, self)._search(cr, user, args, offset=offset, limit=limit, order=order, context=context,
    ...
    def name_search(self, cr, uid, name, args=None, operator='ilike', context=None, limit=100):
        ...
        if name and operator in ('=', 'ilike', '=ilike', 'like', '=like'):
            ...
            cr.execute('''SELECT partner.id FROM product_supplier partner
                          LEFT JOIN product_supplier company ON partner.parent_id = company.id
            ...
            if ids:
                return self.name_get(cr, uid, ids, context)
        return super(product_supplier,self).name_search(cr, uid, name, args, operator=operator, context=context, limit=limit)
    ...
    def view_header_get(self, cr, uid, view_id, view_type, context):
        res = super(product_supplier, self).view_header_get(cr, uid, view_id, view_type, context)
        if res: return res
        if not context.get('category_id', False):
            return False
        return _('Partners: ')+self.pool.get('product.supplier.category').browse(cr, uid, context['category_id'], context).name
    ...
    def _display_address(self, cr, uid, address, without_company=False, context=None):
        ...
        :param address: browse record of the product.supplier to format
